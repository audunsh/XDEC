...
class fragment_amplitudes():
    ...
    def solve_MP2PAO(self, norm_thresh = 1e-10):
        ...
        #print(vp_indx)
        #print(pp_indx)
        f_vir = self.f_mo_aa.cget([0,0,0])
        f_occ = self.f_mo_ii.cget([0,0,0])
        s_vir = self.s_pao.cget([0,0,0])
        sfs_iab = np.einsum("aa,ii,bb->iab",s_vir,f_occ,s_vir)
        sfs_iaj = np.einsum("aa,jj,bb->ajb",s_vir,f_occ,s_vir)
        sf_ab   = np.einsum("aa,bb->ab",f_vir,s_vir)
        fs_ab   = np.einsum("aa,bb->ab",s_vir,f_vir)
        #f_iajb  = sfs_iab[:,:,None,:] - sfs_iaj[:,:,:,None] + sf_ab[None,:,None,:] - fs_ab[None,:,None,:]


        for ti in np.arange(100):
            t2_new = np.zeros_like(self.t2)

            beta = np.zeros_like(self.t2)
            for C in np.arange(self.n_virtual_cells):
                for D in np.arange(self.n_virtual_cells):
                    for J in np.arange(self.n_occupied_cells):
                        ...
                        beta[:,C,:,J,:,D,:] = np.einsum("Kkcjd,Kik->icjd",self.t2[:,C_J[nz],:,J_range[nz],:,D_J[nz],:],Fik)
            ...
            for dL in np.arange(self.n_virtual_cells):
                ...
                for dM in np.arange(self.n_virtual_cells):
                    ...
                    for M in np.arange(self.n_occupied_cells):
                        ...
                        ## Assuming s_pao is stored as Toeplitz matrix similar to Fock matrix
                        ...
                        Fjk = self.f_mo_ii.cget(-pair_extent + pair_extent[M])
                        Sac = self.s_pao.cget(virtual_extent - virtual_extent[dL])
                        Sdb = self.s_pao.cget(virtual_extent - virtual_extent[dM])
                        tnew += np.einsum("Cac,Kjk,iCcKkDd,Ddb->iajb", Sac, Fjk, self.t2[:, :, :, :, :, :, :], Sdb, optimize=True)


                        Sac = self.s_pao.cget(virtual_extent - virtual_extent[dL])
                        Sdb = self.s_pao.cget(virtual_extent - virtual_extent[dM])
                        tnew += np.einsum("Cac,iCcjDd,Ddb->iajb", Sac, beta[:,:,:,M,:,:,:], Sdb, optimize=True)
                        ...
                        #    FKik = self.f_mo_ii.cget(virtual_extent[K])
                        #    CD_t0 = self.d_ia.mapping[self.d_ia._c2i( pair_extent - self.d_ia.coords[K] )]
                        #    nz = (CD_t0<self.n_virtual_cells)*(CD_t0>=0)
                        #    CD_t = CD_t0[nz]
                        #    CD =
